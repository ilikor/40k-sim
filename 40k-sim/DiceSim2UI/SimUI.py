...
class SimUI():
    ...
    def ButtonSimulate(self):
        ...
        try:
            ...
            self.aaa.plot(values, "o", color = self.color_cycle[self.color_number], label = "Shooting")
            
            
            valuesM = numpy.bincount(moral)
            valuesM = valuesM/numpy.sum(valuesM)*100
            
            self.aaa.plot(valuesM, "-", color = self.color_cycle[self.color_number], label = "Plus moral")
